...
;(function(factory) {
    ...
    /*Prueba Gatacca - Creo nuevo metodo para calcular los días por mes*/
    EvoCalendar.prototype.calculateDaysforMonth = function() {
        var _ = this, nameDays, weekStart, firstDay;
        return _.$label.days_in_month[_.$active.month];
    ...
    /*Prueba Gattacca - Creo nuevo metodo para traer los eventos por mes*/
        EvoCalendar.prototype.buildEventListMonthly = function() {
        var _ = this, markup, hasEventToday = false;
        ...
        if (_.options.calendarEvents) {
            for (var i = 0; i < _.options.calendarEvents.length; i++) {
                ...
                if((typeof _.options.calendarEvents[i].date !== 'string')){
                    t = new Date(_.options.calendarEvents[i].date[0]).getMonth() + 1;
                } else{
                    t = new Date(_.options.calendarEvents[i].date).getMonth() + 1;
                }
                if(_.$active.month + 1 === t){
                    eventAdder(_.options.calendarEvents[i])
                }
               /* if(_.isBetweenDates(_.$active.date, _.options.calendarEvents[i].date)) {
                    eventAdder(_.options.calendarEvents[i])
